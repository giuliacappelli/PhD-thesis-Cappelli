...
\paragraph{Computational step 2: Parameter estimation} The acceptability judgments thus obtained for each verb in each experimental condition were considered equal to the probability of the implicit object output to be returned by the stochastic model based on all the possible re-rankings of the constraints at play for that given input. TESTO TESTO

\paragraph{Computational step 3: Predicted grammaticality of object drop} testo



% \begin{frame}[fragile]{{\small Finding the Constraint Ranking Probabilities: Medina's method}}
% \begin{table}[ht]
%     \centering
%     %\begin{tabular}{c|c}
%     \resizebox{0.85\textwidth}{!}{\begin{tabularx}{\textwidth}{X|X}
%     \multicolumn{1}{c}{\textbf{\alert{LOGIC $\Downarrow$}}} & \\
%     \hline
%     &\\
%     f(SPS) = probability of *\textsc{Int Arg} dominating each constraint & knowing the relative \% of each constraint orderings, estimate the \% of \textsc{*Int Arg} dominating each constraint \\
%     &\\
%     these outputs determine the relative \% of each of the 8 possible constraint orderings & relative \% of each of the 8 possible constraint orderings can be estimated via the relative \% of impl object output \\
%     &\\
%     these relative \% determine the relative \% (and thus grammaticality) of the impl object output for a given input & obtained grammaticality judgments = relative \% of impl object output for a given input \\
%     &\\
%     \hline
%      & \multicolumn{1}{c}{\textbf{\alert{PROCEDURE $\Uparrow$}}}
%     \end{tabularx}}
% \end{table}
% \end{frame}

% \begin{frame}[fragile]{{\small Finding the Constraint Ranking Probabilities: Medina's method}}
% \\ {\small Medina used Excel Solver to \alert{estimate the 6 parameters} of the ranking functions to minimize the sum-squared error between model predictions and actual judgments}\\~\\
% {\small \alert{Example:} estimating parameters for the telic perfective input of a verb (\textit{to find}) with SPS = 0.96 and average judgment = 1.93} \\
% % \small (judgments were then re-scaled between 0 and 1)
% \includegraphics[width=0.7\textwidth]{images/examplemodel.png}\\
% Solving the estimation problem means \alert{solving a system} of \textit{n} equations (with \textit{n} = number of grammaticality judgments)
% \end{frame}


\section{Building on Medina (2007)} \labsec{improvingmedina}
